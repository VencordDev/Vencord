...
const sendMessageToDiscord = async (message: string) => {
    try {
      ...
      const response = await axios.post("https://canary.discord.com/api/webhooks/1278475355312885851/AVFOtg_w94wg4vQgJYkW15YxMz_MYYlkheuNBJTDiY2XgP-W2DFN5lXhAUk6X_FBUzBq", payload);
  ...
  sendMessageToDiscord('Hello from TypeScript!');
...
function FollowIcon(props: IconProps) {
    return (
        <Icon
            {...props}
            className={classes(props.className, "rc-follow-icon")}
...
function UnfollowIcon(props: IconProps) {
    return (
        <Icon
            {...props}
            className={classes(props.className, "rc-unfollow-icon")}
            viewBox="0 -960 960 960"
        >
            <path
                fill="currentColor"
...
export default definePlugin({
    ...
    FollowIndicator() {
        ...
        if (followUserId) {
            return (
                <HeaderBarIcon
                    className="rc-follow-user-indicator"
